...
/*   Updated: 2024/03/20 01:25:52 by agaley           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_vars(t_mesh *mesh, t_cone_vars *vars, t_cone_params *params)
{
	...
	vars->perp_axis = cross_product(norm_vec3(params->axis), make_vec3(0, 1,
				0));
	if (vec3_lenght(vars->perp_axis) < 0.0001)
	{
		vars->perp_axis = cross_product(norm_vec3(params->axis), make_vec3(1, 0,
					0));
	}
...
static void	push_verts(t_mesh *mesh, t_cone_params *params, t_cone_vars *vars)
{
	...
	mesh->verts = ft_realloc(mesh->verts, (mesh->verts_count + vars->segments
				* 2 + 1) * sizeof(float) * 3, mesh->verts_count * sizeof(float)
			* 3);
	...
	while (i < vars->segments)
	{
		...
		point_base = add_vec3s(params->center,
				add_vec3s(scale_vec3s(vars->perp_axis, cos(vars->theta)
						* vars->radius), scale_vec3s(vars->perp_axis2,
						sin(vars->theta) * vars->radius)));
		point_top = add_vec3s(point_base, scale_vec3s(norm_vec3(params->axis),
					params->height / 2.0));
		point_base = add_vec3s(point_base, scale_vec3s(norm_vec3(params->axis),
					-params->height / 2.0));
...
static void	push_tris(t_mesh *mesh, t_cone_vars *vars)
{
	...
	while (i < vars->segments)
	{
		...
		next_base_index = vars->verts_start + 1 + ((i + 1) % vars->segments)
			* 2;
		...
		ivec3_to_array(&(t_ivec3){vars->verts_start, base_index,
			next_base_index}, mesh->tris, mesh->tris_count++);
		mesh->tris = ft_realloc(mesh->tris, (mesh->tris_count + 2) * sizeof(int)
				* 3, mesh->tris_count * sizeof(int) * 3);
		ivec3_to_array(&(t_ivec3){vars->verts_start, next_top_index, top_index},
