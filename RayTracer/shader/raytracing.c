...
/*   Updated: 2024/02/21 21:17:36 by agaley           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
t_vec3	trace_ray(t_vec3 origin, t_vec3 direction, int depth, t_uniforms* u)
{
	...
	if (intersect_scene(origin, direction, &hit, u))
	{
		...
		printf("reflect_color: %f %f %f\n", reflect_color.x, reflect_color.y, reflect_color.z);
...
void rt_frag_shader(float* fs_input, t_shader_builtins* builtins, void* uni) {
	...
	fov = tanf(u->cam->fov * (3.1416 / 180.0) / 2); // Convert FOV from degrees to radians and calculate tan
	printf("there is a fragment at %f %f\n", builtins->gl_frag_coord.x, builtins->gl_frag_coord.y);
	printf("there is a fragment at %f %f on screen space\n", builtins->gl_frag_coord.x / WIDTH, builtins->gl_frag_coord.y / HEIGHT);
	float vph = 2 * fov;
	float vpw = aspect_ratio * vph;
	t_vec3 horizon = scale_vec3s(u->cam->dir, vpw);
	t_vec3 vertical = scale_vec3s(u->cam->up, vph);
	t_vec3 horizon_scaled = scale_vec3s(horizon, builtins->gl_frag_coord.x / WIDTH);
	t_vec3 vertical_scaled = scale_vec3s(vertical, builtins->gl_frag_coord.y / HEIGHT);
	t_vec3 lowleft = sub_vec3s(sub_vec3s(sub_vec3s(u->cam->pos, scale_vec3s(horizon, 0.5)), scale_vec3s(vertical, 0.5)), u->cam->dir);
	t_ray	ray = { add_vec3s(u->cam->up, u->cam->pos), add_vec3s(lowleft, add_vec3s(horizon_scaled, sub_vec3s(vertical_scaled, sub_vec3s(u->cam->up, u->cam->pos)))) };

	// Calculate fragment's position in camera space
	frag_coord.x = ((builtins->gl_frag_coord.x) - 0.5) * aspect_ratio * fov;
	frag_coord.y = ((builtins->gl_frag_coord.y) - 0.5) * fov;printf("there is a fragment\ ]n");
	frag_coord.z = -1.0;
