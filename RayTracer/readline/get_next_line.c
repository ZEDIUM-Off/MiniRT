...
/*   Updated: 2024/03/03 19:51:20 by agaley           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_append_line(t_lineread *line, t_buff *buf, ssize_t len)
{
	ssize_t	size;
	...
	line->str = ft_realloc_str(line->str, size * sizeof(char));
	if (!line->str)
		return ;
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0
		|| BUFFER_SIZE > __SIZE_MAX__ / 2 - sizeof(t_buff))
		return (NULL);
	line.cur = 0;
	line.str = NULL;
